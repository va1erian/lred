 ...
 * @author valerian
 */
public class Palette4Codec implements RowCodec {
    ...
    public Level.Row unpackRow(LevelStream stream) {
        ...
        Level.Tile[] palette = {
            Level.Tile.fromByte(stream.pullThreeBits()),
            Level.Tile.fromByte(stream.pullThreeBits()),
            Level.Tile.fromByte(stream.pullThreeBits()),
            Level.Tile.fromByte(stream.pullThreeBits())
        };
        ...
        for(short i = 0; i < Level.ROW_LENGTH; i++) {
            byte tileIndex = stream.pullTwoBits();
            theRow.setTile(i, palette[tileIndex]);
        }
